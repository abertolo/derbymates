...
import src.main.derbymates.test.*
...
@Mock([User,Player,Match,MatchPlayer,Derby,DerbyPlayer])
class PlayerControllerSpec extends Specification {
    ...
    void "test logged user home return his stats"() {
        when:
            Templates.createDerby()
            def user = User.findByUsername('abertolo')

            controller.metaClass.getAuthenticatedUser = {->
                return user
            }

            def map = controller.me()
        then:
            user != null
            map != null
    }
